...
export default function InboxPage() {
  ...
  const Avatar = ({ name, url, jid, size = 49 }) => {
    ...
    const colors = ["#00a884","#0078d4","#e67e22","#9b59b6","#e74c3c","#1abc9c","#f39c12","#2980b9","#16a085","#8e44ad"]
    const seed = name || jid || "?"
    const colorIdx = seed.split("").reduce((acc, ch) => acc + ch.charCodeAt(0), 0) % colors.length
    const initials = seed.replace(/^\+?254|^\+?0/, "").trim().split(/[\s+]/).map(w => w[0]).filter(Boolean).join("").slice(0, 2).toUpperCase() || "?"
    return (
      <div style={{ width: size, height: size, borderRadius: "50%", flexShrink: 0, overflow: "hidden", background: colors[colorIdx], display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#fff", fontSize: size * 0.36, userSelect: "none" }}>
        {finalUrl
